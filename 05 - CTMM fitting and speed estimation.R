...
# Script: 05 - CTMM fitting and speed estimation
...
# "GPS" tracks
all.tracks <- readRDS(paste0(getwd(),
                             "/data_derived/sampled_tracks/tracks_",
                             which.tracks, 
                             ".rds"))
...
# (6) calculate the diffusion-based speed estimate
  ...
  for (i in 1:length(unique(df$indiv))) {
    ...
    for (j in 1:nrow(scenarios)) {
      ...
      if (names(fitted.mods)[1] %in% c("OUF",
        ...
        # mean speed of Gaussian movement process (m/s)
      ...
      # diffusion-based speed estimation (cannot be IID)
      if (names(fitted.mods)[1] %in% c("OU",
                                       "OU anisotropic",
                                       "OUF",
                                       "OUF anisotropic",
                                       "OUf",
        ...
        focal.i.scenario$rms.speed.hi <- sqrt(4 * diff.ests[3] / focal.scenario$rate %#% "hour")
        
      } else {
        
        focal.i.scenario$rms.speed.lo <- NA
        focal.i.scenario$rms.speed.est <- NA
        focal.i.scenario$rms.speed.hi <- NA
    ...
    if (i %% 10 == 0) {
      ...
      # write file
      saveRDS(all.speeds, paste0(getwd(), "/data_derived/sampled_ctmm_speeds/", which.tracks, ".rds"))
...
# takes 6-11 minutes for one individual
