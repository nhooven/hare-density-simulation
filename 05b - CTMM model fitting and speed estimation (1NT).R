...
# Script: 05b - CTMM model fitting and speed estimation (1NT)
...
# "GPS" tracks
all.tracks <- read.csv("Derived data/Sampled - GPS tracks/tracks_1NT.csv")
...
ctmm_speeds <- function (df) {
  ...
  for (i in 1:length(unique(df$indiv))) {
    ...
    if (i %% 10 == 0) {
      ...
      # write .csv
      write.csv(all.speeds, file = paste0(getwd(), "/Derived data/Sampled - CTMM speeds/ctmm_speeds_1NT.csv"))
...
write.csv(ctmm.speeds, file = paste0(getwd(), "/Derived data/Sampled - CTMM speeds/ctmm_speeds_1NT.csv"))
