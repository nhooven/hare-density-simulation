...
# Script: 05c - Track interpolation and camera contacts (simple - high)
...
sims.df <- read.csv(paste0(getwd(), "/Derived_data/Simulated data/sims_simple_high.csv"))
...
# passes
write.csv(all.passes, paste0(getwd(), "/Derived_data/Passes/passes_simple_high.csv"))

# ctmms (do we need to do this if we're not running speed estimates?)
save(all.ctmms, file = paste0(getwd(), "/Derived_data/CTMMs/ctmms_simple_high.RData"))
