...
# Script: 16a - Assess and visualize REMs (all)
...
all.rem.boot <- read.csv(paste0(getwd(), "/Derived_data/REM results/all_rem_boot.csv"))
...
# before
ggplot(data = all.rem.boot.1 %>% filter(trt == "before")) +
...
# after
ggplot(data = all.rem.boot.1 %>% filter(trt == "after")) +
...
# before
ggplot(data = all.rem.boot.2 %>% filter(trt == "before")) +
...
# after
ggplot(data = all.rem.boot.2 %>% filter(trt == "after")) +
...
all.rem.boot.cv.summary <- all.rem.boot.2 %>%
  ...
  group_by(trt, n.cams, n.indiv, method) %>%
...
# grouped column plot - before
ggplot(data = all.rem.boot.cv.summary %>% filter(trt == "after")) +
...
all.rem.cov <- all.rem.boot.2 %>%
  ...
  group_by(trt, method, n.cams, true.density) %>%
